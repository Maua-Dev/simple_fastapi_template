...
class Test_Item:
    def test_item(self):
        item = Item("test", 1.0, ItemTypeEnum.FOOD)
    ...
    def test_item_name_is_none(self):
        with pytest.raises(ParamNotValidated):
            Item(price=1.0, item_type=ItemTypeEnum.FOOD)
            
    def test_item_name_is_not_string(self):
        with pytest.raises(ParamNotValidated):
            Item(name=1.0, price=1.0, item_type=ItemTypeEnum.FOOD)
            
    def test_item_name_is_too_short(self):
        with pytest.raises(ParamNotValidated):
            Item(name="te", price=1.0, item_type=ItemTypeEnum.FOOD)
            
    def test_item_price_is_none(self):
        with pytest.raises(ParamNotValidated):
            Item(name="test", item_type=ItemTypeEnum.FOOD)
    ...
    def test_item_type_is_none(self):
        with pytest.raises(ParamNotValidated):
            Item(name="test", price=1.0)
            
    def test_item_type_is_not_enum(self):
        with pytest.raises(ParamNotValidated):
            Item(name="test", price=1.0, item_type="FOOD")
